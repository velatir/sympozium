...
// just began. Used when a sidecar issues clearContext() between
...
// Prompt issues a single Anthropic Messages call on
// behalf of a sidecar. With useContext false the message slice is
// temporarily reset for the call (and restored via defer) so the answer
// is stateless. With useContext true the prompt is appended and the
// assistant reply recorded so context grows across Prompt calls within
// the loop. Tool-use is suppressed via tool_choice=none — the model is
// expected to answer in text only; structured output (when Schema is
// set) is parsed by the caller from the assistant's text block.
