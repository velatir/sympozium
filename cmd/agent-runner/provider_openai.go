...
// ResetContext rebuilds the message slice to the seed
// state so the next Chat or Prompt call behaves as if the conversation
// just began. Used by the sidecar-initiated clearContext IPC between
// independent units of work so token accumulation does not leak across
// boundaries.
...
// Prompt issues a single LLM call on behalf of a
// sidecar. When useContext is false the call is stateless: messages are
// temporarily reset to [system, prompt] for the call and restored on
// exit. Otherwise the prompt is appended and the assistant reply
// recorded, so the conversation history grows across Prompt calls within
// the loop. Schema is forwarded via OpenAI's response_format json_schema
// when provided. The first return is the raw text; the second is the
// parsed JSON payload when Schema was set and the model emitted JSON.
